#Name:mantu kumar
#Roll no:-EE19B039
import sys            #importing module sys
import numpy as np    #importing numpy as np
import cmath          #importing module cmath
...
class I_S:
    ...
    def __init__(self,node1,node2,val,phi):
        self.nodea = int(node1)
        self.nodeb = int(node2)
        self.val = cmath.rect(val,(Pi*phi)/180)
    ...
    if line[0][0] == "R":
        ...
        if sorted([nodes[line[1]],nodes[line[2]]]) not in Shorts:
            if line[3].lower() == "dc":
                ...
                V_sources[line[0]] = Voltage(nodes[line[1]], nodes[line[2]], float(line[4]), 0.0)
            elif line[3].lower() == "ac":
                print("I got here in ac!")
                V_sources[line[0]] = Voltage(nodes[line[1]], nodes[line[2]], float(line[4])/2, float(line[5]))
    elif line[0][0] == "I":
        if sorted([nodes[line[1]],nodes[line[2]]]) not in Shorts:
            if line[3].lower() == "dc":
